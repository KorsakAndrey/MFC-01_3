...
void GyverTM1637::point(int n, boolean PointFlag)
{
    if (PointFlag) PointData[n] = 0x80;
	else PointData[n] = 0;
	update();
}
// ************************** ВСЯКИЕ ФУНКЦИИ *****************************
...
void GyverTM1637::displayFloat(float value){
    if (value > 999.9 || value < -99.9) return;
	boolean negative = false;
	boolean neg_flag = false;
	byte digits[4]={1, 2, 3, 4};
	if (value < 0) negative = true;
	value = abs(value);
	digits[0] = (int)value / 100;      	// количесто сотен в числе
	float b = (int)digits[0] * 100; 	// вспомогательная переменная
	digits[1] = ((int)value - b) / 10; 	// получем количество десятков
	b += digits[1] * 10;               	// суммируем сотни и десятка
	digits[2] = (int)value - b;  	// получем единицы
	b += digits[2];                	// сумма сотен и десятков и едениц
	digits[3] = int((value * 10) - (b * 10)); // десятая доля
	b = b * 10 + digits[3];
    byte hung = (value * 100) - (b * 10); //сотая доля
	...
	digits[2] += 128;
	if((value < 100 && !negative) || (value < 10 && negative)) {
        for(byte i = 0; i < 3; i++) {
            digits[i] = digits[i+1];
        }
         digits[3] = digToHEX(hung);
    }
...
void GyverTM1637::twistByte(uint8_t BitAddr, uint8_t DispData, int delayms) {
	...
	//for (byte r = 0; r < 1; r++) {
	for (byte i = 5, j = 0; i < 11; i++, j++) {
		step |= (1 << i%6)  ;// бегает дырка
		step &=~(1 << j);
		displayByte(BitAddr, step);
		delay(delayms);
	}
